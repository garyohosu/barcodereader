...
### Q14. 「二度と表示しない」選択後の権限エラー画面の挙動【重大度: 中】⏳

シーケンス3では権限拒否時に「スタート画面にエラーメッセージ表示」としているが、  
Androidでは「二度と表示しない」を選択すると以降ダイアログが出なくなる。

この場合、ユーザーはOSの設定画面から手動で権限を有効化する必要がある。

**確認事項:**
- エラーメッセージに「設定 → アプリ → 権限から許可してください」のような案内文を出すか
- 設定画面へ直接遷移するボタンを用意するか（`Intent(Settings.ACTION_APPLICATION_DETAILS_SETTINGS)`）
- それとも「スタートボタンで再試行」の文言だけで今回はよいか

---

### Q15. 「もう一度」後のカメラ挙動【重大度: 中】⏳

シーケンス6では判定画面 → もう一度 → 読み取り画面の流れで「カメラ再起動」と記載したが、  
判定画面ではカメラが停止しているか継続しているかが仕様上未定義。

**確認事項:**
- 判定画面ではカメラを停止するか（バッテリー節約）、それともバックグラウンドで継続するか
- 停止する場合: 「もう一度」でカメラを再起動する → 起動待ち時間が生じる
- 継続する場合: 「もう一度」で即座に読み取り再開できる → 判定中に意図しないスキャンが入るリスクあり

---

### Q16. BarcodeAnalyzerのスレッドとViewModelへの通知方法【重大度: 低】⏳

CameraXの `ImageAnalysis` コールバック（`analyze()`）はバックグラウンドスレッドで実行される。  
ViewModel の StateFlow を更新するには Main スレッドへの切り替えが必要。

**確認事項:**
- `withContext(Dispatchers.Main)` で切り替えるか
- `ImageAnalysis.setAnalyzer` に `Dispatchers.Main` を渡すか
- 実装者判断でよいか（実装詳細として保留）
...
| 権限拒否後の再試行 | 再スタートで再要求。再表示不可なら設定許可を案内 |
| UIテスト | 今回は対象外 |
